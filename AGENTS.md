...
- **Voice** `/v1/voice/*` require key (cloud path): `GET voices`, `POST stt` (Whisper, needs `OPENAI_API_KEY`), `POST ask` (audio→answer→mp3 base64), `POST tts` (EdgeTTS vi-VN, no key, `edge-tts` pip); audio never logged; `VOICE_*` keys in `cloud/.env.example`; `voice_max_audio_mb` → 413.
